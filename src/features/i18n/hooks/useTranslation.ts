    ...
    try {
      ...
      if (result === key && process.env.NODE_ENV === 'development') {
        ...
        if (similarKeys.length > 0) {
          i18nLogger.warn(`   Similar keys found: ${similarKeys.slice(0, 3).join(', ')}`);
        }
      }
      
      // Ensure the return type is always a string for React components
      if (typeof result === 'object' && result !== null && !options?.returnObjects) {
        i18nLogger.warn(`Translation for key "${key}" returned an object without 'returnObjects' option. Returning key as fallback.`);
        return key;
      }
      ...
      i18nLogger.error(`Translation error for key "${key}":`, error);
